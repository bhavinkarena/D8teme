...
const uploadPost = asyncHandler(async (req, res) => {
  ...
  if (!cloudinaryResponse) {
    return res
      .status(500)
      .json(
        new ApiError(500, null, "Something went wrong while uploading the file")
      );
  }
  console.log(userId);
  ...
  res.json(new ApiResponse(200, newPost, "Post Upload Successful 👍"));
});
...
const countLikes = asyncHandler(async (req, res) => {
  ...
  res.json(
    new ApiResponse(200, { likesCount: post.likes.length }, "Likes counted")
  );
});
